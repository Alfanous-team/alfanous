...
class Raw():
	...
	def _do(self,flags):
		  ...
		  id=flags["id"] if flags.has_key("id") else self._defaults["flags"]["id"]
	...
	def _init_stats(self):
	     ...
	     stats={}
	     for id in ["TOTAL"]: #self._ids.extend(["TOTAL"])
		  stats[id]={}
		  stats[id]["total"]=0
		  stats[id]["other"]={}
		  stats[id]["other"]["total"]=0
		  for action in self.DOMAINS["action"]:
		      stats[id][action]={}
		      stats[id][action]["total"]=0
		      stats[id][action]["other"]={}
		      stats[id][action]["other"]["total"]=0
		      for flag,domain in self.DOMAINS.items():
			    stats[id][action][flag]={}
			    stats[id][action][flag]["total"]=0
			    stats[id][action][flag]['other']=0
			    for val in domain:
				stats[id][action][flag][str(val)]=0
	...
	def _process_stats(self,flags):
	     ...
	     stats=self._stats
	     #Incrementation 
	     for id in ["TOTAL"]: #["TOTAL",flags[id]]
		    stats[id]["total"]+=1
		    action=flags["action"]
		    if action in self._domains["action"]:
			      stats[id][action]["total"]+=1
			      for flag,val in flags.items():
				    if flag in self._domains.keys(): 
						stats[id][action][flag]["total"]+=1
						if val in self._domains[flag]: 
						      stats[id][action][flag][str(val)]+=1
						else: 
						      stats[id][action][flag]["other"]+=1
				  
				    else: 
						stats[id][action]["other"]["total"]+=1
				    

		    else: stats[id]["other"]["total"]+=1 
