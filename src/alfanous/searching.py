...
def _collect_plugin_expanded_terms(q):
    ...
    standard ``Term`` queries targeting ``aya_fuzzy``, ``lemma_norm``, or
    ...
    _DERIV_FIELDS = frozenset({"aya_fuzzy", "lemma_norm", "aya_root"})
...
class QSearcher:
    ...
    def search(self, querystr, limit=QURAN_TOTAL_VERSES, sortedby="score", reverse=False, facets=None, filter_dict=None, fuzzy=False, fuzzy_maxdist=1, derivation_level=0, timelimit=5.0):
        ...
        _DERIV_FIELD_MAP = {
            ...
            2: ("lemma_norm",    "lemma"),     # lemma — lookup key, search field
            ...
            if _deriv_info is not None:
                ...
                if _deriv_field in self._schema:
                    if _deriv_key is None:
                        ...
                        # Key values are vocalized (e.g. "مُلْك") but the target
                        # field uses QStandardAnalyzer which strips vocalization,
                        # so we normalize each value through the field's analyzer.
