...
class QSearcher:
    ...
    def search(self, querystr, limit=QURAN_TOTAL_VERSES, sortedby="score", reverse=False, facets=None, filter_dict=None, fuzzy=False, fuzzy_maxdist=1, derivation_level=0, timelimit=5.0):
        ...
        #
        # The field-based approach searches a pre-indexed field where each
        # word position stores its lemma or root value, rather than expanding
        # query terms to all morphological forms at query time.
        ...
        if _deriv_level_int >= 1 and not _has_wildcard_query(query):
            _deriv_info = _DERIV_FIELD_MAP.get(_deriv_level_int)
            if _deriv_info is not None:
                _deriv_field, _deriv_key = _deriv_info
                if _deriv_field in self._schema:
                    if _deriv_key is None:
                        # Level 1 (stem): parse query with aya_fuzzy parser.
                        if self._fuzzy_parser is None:
                            self._fuzzy_parser = _QueryParser("aya_fuzzy", schema=self._schema)
                        try:
                            stem_query = self._fuzzy_parser.parse(querystr)
                            if stem_query is not None:
                                stem_query = _strip_phrase_queries(stem_query)
                                derivation_subqueries.append(stem_query)
                                for ft, ftext in stem_query.all_terms():
                                    _derivation_expansion.add((ft, ftext))
                        except StopIteration:
                            pass
                    else:
                        # Level 2/3 (lemma/root): look up key values for each
                        ...
                        for _fieldname, term in query.all_terms():
                            ...
                            key_values = _lookup_key_values(term, _deriv_key)
                            for kv in key_values:
                                if kv:
                                    derivation_subqueries.append(wquery.Term(_deriv_field, kv))
                                    _derivation_expansion.add((_deriv_field, kv))
                else:
                    # Fallback: target field not in the index — use old
                    # expansion approach for backward compatibility.
                    from alfanous.query_plugins import DerivationQuery
                    # Map new levels to old DerivationQuery levels:
                    # 1 (stem) → old level 1, 2 (lemma) → old level 1, 3 (root) → old level 2
                    old_deriv_level = 2 if _deriv_level_int >= 3 else 1
                    plugin_expanded = _collect_plugin_expanded_terms(query)
                    plugin_expanded_words = {term for _, term in plugin_expanded}
                    seen_derivation_terms = set()
                    for _fieldname, term in query.all_terms():
                        ...
                        if term in plugin_expanded_words:
                            continue
                        seen_derivation_terms.add(term)
                        derivations = DerivationQuery._get_derivations(term, old_deriv_level)
        ...
        if fuzzy:
            ...
            # skipped.
            # NOTE: fuzzy no longer searches aya_fuzzy for stemming — stemming
            # is controlled exclusively via derivation_level.  The aya_fuzzy
