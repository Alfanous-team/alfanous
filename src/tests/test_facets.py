...
def test_invalid_facet_field():
    """Test that invalid facet fields are handled gracefully."""
    search_flags = {
        "action": "search",
        "query": "الله",
        "facets": "invalid_field"
    }
    
    results = RAWoutput.do(search_flags)
    
    # Should not crash and should return successful search
    assert results["error"]["code"] == 0
    
    # Invalid facet should be ignored
    if "facets" in results["search"]:
        assert "invalid_field" not in results["search"]["facets"]


def test_mixed_valid_invalid_facets():
    """Test that valid facets work even when mixed with invalid ones."""
    search_flags = {
        "action": "search",
        "query": "الله",
        "facets": "sura_id,invalid_field,juz"
    }
    ...
    # Valid facets should be present
    assert "facets" in results["search"]
    assert "sura_id" in results["search"]["facets"]
    assert "juz" in results["search"]["facets"]
